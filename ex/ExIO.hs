...
when :: Bool -> IO () -> IO ()
when b ax = if b then ax else skip
...
zipWithIO_ f xs = sequenceIO_ . zipWith f xs
...
replicateIO n = sequenceIO . genericReplicate n

replicateIO_ :: Integral i => i -> IO a -> IO ()
replicateIO_ n = sequenceIO_ . genericReplicate n
